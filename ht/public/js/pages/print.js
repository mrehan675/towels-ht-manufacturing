...
frappe.ui.form.PrintView = class {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${frappe.render_template('print_skeleton_loading')}
				<iframe class="print-format-container" width="150%" height="0" frameBorder="0" scrolling="yes">
				</iframe>
			</div>
			<div class="page-break-message text-muted text-center text-medium margin-top"></div>
		</div>`
		...
		// Add CSS to center the iframe
		this.print_wrapper.find('.iframe-container').css({
			display: 'flex',
			justifyContent: 'center', // Center horizontally
			overflow: 'hidden', // Prevent horizontal scrolling
		});
	
		this.print_wrapper.find('.print-format-container').css({
			marginLeft: '-15%', // Adjust for the extra width
			marginRight: '-15%', // Adjust for the extra width
			marginTop: '-10%', // Adjust for the extra width
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = frappe.dom.pixel_to_inches(print_height);
			console.log("print_height_inches",print_height_inches);
			// if contents are large enough, indicate that it will get printed on multiple pages
			// Maximum height for an A4 document is 11.69 inches
			if (print_height_inches > 11.69) {
				$message.text(__('This may get printed on multiple pages'));
	...
	// 	// Custom CSS to override padding and other styles for the print page
	// 	const custom_css = `
	...
	// 		}
	// 		.print-format {
	// 			padding: 0 !important; /* Remove padding from the print format */
	// 			margin: 0 !important; /* Remove margin */
	// 		}
	// 		body {
	// 			padding: 0 !important; /* Remove padding from the body */
	// 			margin: 0 !important; /* Remove margin */
	// 		}
	// 	`;
	...
	// 		`<style type="text/css">${out.style}</style>
	// 		<link href="${print_css_url}" rel="stylesheet">
	// 		<style>${custom_css}</style>` // Inject custom CSS
	// 	);
